...
# Create an S3 bucket with Object Lock enabled (requires enabling at create time) and versioning on.
# Usage: ./s3-create-object-lock.sh [-p profile] [-r region] <bucket-name>
...
echo "Creating Object Lock bucket: $BUCKET"
create_bucket "$BUCKET" "${AWS_REGION:-${AWS_DEFAULT_REGION:-}}" true
wait_bucket "$BUCKET"
enable_versioning "$BUCKET"
set_public_access_block_all "$BUCKET"
echo "Done. You can now set lock configuration (governance/compliance) with put-object-lock-configuration."
